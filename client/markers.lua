					...
					if v.propertyEntering ~= nil then 
						...
						if #(pCoords - vector3(v.propertyEntering.x, v.propertyEntering.y, v.propertyEntering.z)) < 1.2 then
							...
							if IsControlJustPressed(1, 51) then
								TriggerServerEvent("fProperty:OwnedProperties", v.propertyID)
							end                                
						end  
					end
					if v.propertyGarage ~= nil then 
						if v.propertyOwner == "-" or v.propertyOwner == b.identifier then 
							...
							if #(pCoords - vector3(v.propertyGarage.x, v.propertyGarage.y, v.propertyGarage.z)) < 1.2 then
								ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour intéragir.")
								if IsControlJustPressed(1, 51) then
									TriggerServerEvent("fProperty:OwnedGarages", v.propertyID)
								end                                
							end 
						end
					end
							...
							if #(pCoords - vector3(v.propertyRented.x, v.propertyRented.y, v.propertyRented.z)) < 1.2 then
								...
								if IsControlJustPressed(1, 51) then
									if IsPedSittingInAnyVehicle(PlayerPedId()) then 
										TriggerServerEvent("fProperty:AddVehicleIntoGarage", v.propertyID, getVehicleProps(GetVehiclePedIsIn(PlayerPedId(), false)), GetVehicleLabel(GetEntityModel(GetVehiclePedIsIn(PlayerPedId(), false))))
									else
					...
					for t,b in pairs(Config.GarageInteriors) do 
						...
						if not InVisit then 
							...
							if #(pCoords - b.Computer) < 1.2 then
								...
								if IsControlJustPressed(1, 51) then
									TriggerServerEvent("fProperty:GetVehiclesProperties", v.propertyID)
								end                                
							end
						end
					end
				...
				for k,v in pairs (myProperties) do 
					for t,b in pairs (UsersInfos) do
						if v.propertyOwner == b.identifier then  
							...
							if IsControlJustPressed(0, 56) then
								TriggerServerEvent("fProperty:OpenOwnerList", v.propertyID)
							end
						end
					end
					for _, interior in pairs(Config.PropertiesInteriors) do 
						...
						if #(pCoords - interior.Exit) < 1.2 then
							ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour intéragir.")
							if IsControlJustPressed(1, 51) then
								TriggerEvent("fProperty:OpenGestionMenu", v.propertyID)
							end                                
						end 
						if not InVisit then 
							...
							if #(pCoords - interior.Storage) < 1.2 then
								...
								if IsControlJustPressed(1, 51) then
									TriggerEvent("fProperty:OpenStorageMenu", v.propertyID)
								end                                
							end 
						end
					end
