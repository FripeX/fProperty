...
SellProperty = function(propertyID, propertyPlayer)
    TriggerServerEvent("fProperty:GivePropertyToPlayer", propertyID, GetPlayerServerId(propertyPlayer))
end
...
SetPlayerIntoBucket = function(Actions)
    if Actions == true then 
        for k,v in pairs(myProperties) do 
            TriggerServerEvent("fProperty:setPlayerToBucket", v.propertyID)
        end
    else
        TriggerServerEvent("fProperty:setPlayerToNormalBucket")
    end
end
...
Announce = function(Index)
    if Index ~= 3 then
        TriggerServerEvent("fProperty:Announce", Index, nil)
    else
        local Reason = Input("Announce", "Annonce (75 caractères max): ", "", 75)
        TriggerServerEvent("fProperty:Announce", Index, Reason)
    end
end
...
EnterProperty = function(propertyType, propertyID, isVisit)
    TriggerServerEvent("fProperty:EnteringInteract", propertyType, propertyID, isVisit)
end

ExitProperty = function(propertyID)
    TriggerServerEvent("fProperty:ExitInteract", propertyID)
end

RegisterNetEvent("fProperty:ExitProperty")
AddEventHandler("fProperty:ExitProperty", function(propertyID)
    for k,v in pairs (myProperties) do 
        if v.propertyID == propertyID then 
            DoScreenFadeOut(500)
            Wait(500)
...
RegisterNetEvent("fProperty:EnteringProperty")
AddEventHandler("fProperty:EnteringProperty", function(propertyType, propertyID, isVisit)
...
ExitGarageWithVehicle = function(CurrentVehicle)
    ...
    for k,v in pairs(myProperties) do 
        ...
        if InGarage == true then 
            ...
            InGarage, myProperties, myVehicles, myOwnedVehicles, myGarageVehicles = false, {}, {}, {}, {}
            TriggerServerEvent("fProperty:UpdateStored", CurrentVehicle.plate, GetVehicleLabel(exitVehicle.model))
        end
    end
    Wait(1000)
    DoScreenFadeIn(1000) 
...
RegisterNetEvent("fProperty:refreshProperty")
AddEventHandler("fProperty:refreshProperty", function(userData, propertiesData)
...
RegisterNetEvent("fProperty:reloadVehicles")
AddEventHandler("fProperty:reloadVehicles", function(vehicleData, ownedData)
...
RegisterNetEvent("fProperty:reloadInfos")
AddEventHandler("fProperty:reloadInfos", function(propertyData)
...
RegisterNetEvent("fProperty:checkGarage")
AddEventHandler("fProperty:checkGarage", function()
    local pVehicle = GetVehiclePedIsIn(PlayerPedId())
    DeleteEntity(pVehicle)
end)

RegisterNetEvent("fProperty:DringDringReponse")
AddEventHandler("fProperty:DringDringReponse", function(Sonnerie)
    GetInvite = Sonnerie
    if not InProperty then 
        for k,v in pairs(GetInvite) do 
            TriggerServerEvent("fProperty:NotInProperty", v.source)
        end
        GetInvite = {}
        return 
    end
...
RegisterNetEvent("fProperty:ViewInteriors")
AddEventHandler("fProperty:ViewInteriors", function(bool)
    if bool == true then 
        InView = true 
    else
        InView = false 
